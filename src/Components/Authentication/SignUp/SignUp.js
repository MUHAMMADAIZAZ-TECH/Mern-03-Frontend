...
const SignUp = () => {
  const validationSchema = Yup.object().shape({
    FirstName: Yup.string().required('First name is required').min(2, 'Too Short!').max(50, 'Too Long!').matches(/^[A-Za-z]+$/,'Firstname can only contain alphabets'),
    LastName: Yup.string().required('Last name is required').min(2, 'Too Short!').max(50, 'Too Long!').matches(/^[A-Za-z]+$/,'Lastname can only contain alphabets'),
